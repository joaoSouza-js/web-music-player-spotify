...
export function Footer() {
    ...
    const { handleVolumeChange, musicVolume, handleMuteApplicationSpeaker } = useMusicPlayerControl({ audioRef })
    ...
    return (
        <footer className="bg-zinc-800 border-t-zinc-700 p-4 border-t items-center justify-between flex h-[15vh] ">
            ...
            <div className="flex items-center gap-2  min-w-[250px]  h-full ">
                {
                    currentMusic && (
                        <Image
                            alt="capa do album hero and vilans do artista metro bomming"
                            src={currentMusic?.photo ?? ""}
                            height={80}
                            width={72}
            ...
            <div className="flex-1 flex  items-center flex-col">
                <div className="flex  gap-6 items-center justify-center   ">
                    ...
                    <button
                        ...
                        className={`bg-white h-10 w-10 items-center ${!musicIsAlreadyPlaying && "pl-1"} justify-center  flex rounded-full text-black`}
                    >   {
                            musicIsAlreadyPlaying === false ? (
                ...
                <div className="flex gap-2 mt-2">
                    <span className="text-sm text-zinc-400">{timeFormatter(musicPlayedTime)}</span>
                    <Slider
                        defaultValue={[0]}
                        onValueCommit={handleReturnToPlayMusic}
                    ...
                    />
                    <span className="text-sm text-zinc-400">{timeFormatter(audioRef.current?.duration)}</span>
                </div>
